...
| **2026-08 초** | (작업 없이) 심사 결과 도착 | **반려** — 7월 3일자 40페이지 보강분이 평가 대상이었음 |
| 07-03 ~ 09-08 | 작업 없음 (2개월 방치) | — |
| **09-08** | Phase 0·1·2 전체 수행. 도구 111개 전부 1,500자+, 허위 설명 10건 정정, EN 색인 제외, Bing·네이버 등록 | 심사 요청 예정 (09-15) |
...
Phase 2 — 도구 페이지  ✅ 2026-09-08 완료
[x] 배치 A (35개) — 500~800자
[x] 배치 B (35개) — 800~1,100자
[x] 배치 C (12개) — 1,100~1,500자
[x] 최종: KO 도구 111개 전부 1,500자 이상. 평균 1,394자 → 2,310자, 텍스트 비율 2.3% → 3.7%
[x] 허위 구현 설명 총 10건 정정 (아래 목록)
[x] scripts/auto/audit-claims.js 로 본문-코드 대조 상시 검증 (현재 불일치 0)

### 정정한 허위 구현 설명 10건

콘텐츠 작업 중 본문이 사실과 다른 설명을 하고 있던 페이지를 찾아 모두 고쳤다.
분량을 늘리는 것보다 이쪽이 신뢰도에 더 직접적인 영향을 준다고 판단했다.

| 페이지 | 적혀 있던 것 | 실제 |
|---|---|---|
| `life/speech-to-text` | "서버 업로드 없이 브라우저에서 안전하게 처리" | webkitSpeechRecognition. 크롬 계열에서 음성이 외부 전송됨 |
| `life/lottery-generator` | "최대 10게임 생성", "특정 번호 제외 옵션" | 두 기능 모두 UI에 없음 |
| `life/stopwatch` | "performance.now()와 requestAnimationFrame 사용" | 코드에 0회. Date.now + setInterval(10ms) |
| `life/coin-flip` | "crypto.getRandomValues()로 암호학적 난수" | Math.random() |
| `life/roulette` | "crypto.getRandomValues()로 난수 생성" | Math.random() |
| `life/pomodoro-timer` | "localStorage에 진행 상황 저장" | 저장 안 함 |
| `life/countdown-timer` | "Performance API로 정확한 시간 측정" | 해당 API 없음 |
| `life/screen-recorder` | "VP8/VP9 또는 H.264(MP4) 코덱" | vp9 하드코딩. MP4 안 나옴 |
| `life/ocr` | "중국어 등 100개 이상 언어 지원" | 한국어·영어·일본어뿐 |
| `life/ai-detector` | "AI 생성 확률"을 백분율로 제시 | 문장 길이 분산 + 단어 빈도 휴리스틱. 판별 불가 |

`ai-detector`는 화면 라벨과 locales까지 함께 고쳤다. 도구 자체의 존치 여부는 별도 판단 사항으로 남긴다.

### 검산으로 잡은 수치 오류 5건

집필 후 계산해 확인한 것들. 숫자를 쓸 때는 반드시 검산할 것.

- 3d6에서 합 10이 합 3보다 드문 배수: 50배 → **27배** (216가지 전수 계산)
- 동전 100회에 5연속 발생 확률: 80% → **97.2%** (몬테카를로 20만 회)
- 2023년 동전 던지기 검증 실험: 4만 8천 번 → **48명이 35만 번 이상**
- 소문자 16자 비밀번호 엔트로피: 43비트 → **75비트** (예시 논리 자체가 뒤집혀 문단 재작성)
- 94^12 조합 수: "수천조" → **10의 23제곱 규모**

Phase 3 — 검토 요청
[x] Search Console 색인 요청 (14개) — 09-08
[x] sitemap 재제출 — 09-08
[x] Bing 웹마스터도구 등록 + URL 100건 제출 — 09-08
[x] 네이버 서치어드바이저 소유확인 + sitemap 제출 — 09-08
[ ] **09-15 애드센스 검토 요청** (색인 요청분이 크롤링될 시간을 준 뒤)
[ ] 결과를 이 문서 §2 이력 표에 기록  ← 다음 사이클을 위해

### 09-15 심사 요청 시 판단 기준

반려되더라도 **사유가 바뀌었는지**를 먼저 본다.

- **사유가 달라졌다** → 콘텐츠 문제는 해결된 것. 새 사유에 맞춰 대응
- **또 "가치가 별로 없는 콘텐츠"** → 콘텐츠가 원인이 아니라는 뜻.
  도메인 신뢰도와 외부 유입 쪽으로 방향 전환 필요.
  (신생 도메인 + 외부 링크 사실상 0 + 도구 사이트라는 포화 카테고리 조합)

참고: 애드센스는 재신청 횟수 제한이나 불이익이 없다. 무한정 기다릴 이유는 없다. 반드시
```
